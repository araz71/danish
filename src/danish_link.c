    ...
    if (packet->dst == danish_address) {
        ...
        if (packet->function == FUNC_WRITE) {
            ...
            if (reg->rwaddr == packet->src && (reg->flags & write_ack_flag)) {
                ...
                if (reg->write_ack_data != NULL) {
                	if (reg->write_ack_size == 255 || reg->write_ack_size == packet->len) {
                		memcpy(reg->write_ack_data, packet->data, packet->len);
                	}
                }
            }
            ...
            if (reg->flags & 0x3) {
                if (reg->flags & read_flag) {
                    ...
                    tx_len = danish_make(danish_address, reg->rwaddr, FUNC_READ,
                                             reg->bregID, 0, NULL, tx_buffer);

                } else {
                    reg->flags |= write_ack_flag;
                    reg->flags &= ~write_flag;
                    tx_len = danish_make(danish_address, reg->rwaddr, FUNC_WRITE,
                                             reg->bregID, reg->size, reg->ptr, tx_buffer);
                }

                if (tx_len != 0) {
                    danish_writer(tx_buffer, tx_len);	// Two-bytes extra for SR pin
                    tx_len = 0;
                }
            }
        }
